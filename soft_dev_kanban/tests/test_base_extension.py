...
class TestBaseExtension(common.SingleTransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.backlog = cls.stage_model.create(
            {'name': 'TEST BACKLOG', 'stage_type': 'backlog'})
        cls.input = cls.stage_model.create(
            {'name': 'TEST INPUT', 'stage_type': 'queue'})
        cls.queue = cls.stage_model.create(
            {'name': 'TEST BUFFER', 'stage_type': 'queue'})
        cls.analysis = cls.stage_model.create(
            {'name': 'TEST ANALYSIS', 'stage_type': 'analysis'})
        cls.dev = cls.stage_model.create(
            {'name': 'TEST DEVELOPMENT', 'stage_type': 'dev'})
        cls.review = cls.stage_model.create(
            {'name': 'TEST REVIEW', 'stage_type': 'review'})
        cls.done = cls.stage_model.create(
            {'name': 'TEST DONE', 'stage_type': 'done'})
